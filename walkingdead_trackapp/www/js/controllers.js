...
.controller('trackDetailCtrl', ['$scope', '$http', '$stateParams', '$timeout', // The following is the constructor function for this page's controller. See https://docs.angularjs.org/guide/controller
...
function ($scope, $stateParams, $state, $q, $http, $filter, $timeout) {
    ...
    function TrackUpdate() {        
        $http({
            method: "GET",
            url: "http://beta3.irealtor.api.iproperty.com.my/smarttrack/tracks/refresh",
            headers: { 'Content-Type': 'application/json' }
        })
        .success(function (response) {           
            console.log("$scope.tracklist : " + JSON.stringify(response));
